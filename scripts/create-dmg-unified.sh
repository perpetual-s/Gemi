...
# Gemi DMG Creator - Unified, Reliable, Beautiful
...
find_or_build_app() {
    ...
    local search_paths=(
        ...
        "$HOME/Library/Developer/Xcode/DerivedData/*/Build/Products/Release/Gemi.app"
        "$HOME/Library/Developer/Xcode/DerivedData/*/Build/Products/Debug/Gemi.app"
    )
    
    for path in "${search_paths[@]}"; do
        if [[ -d $(echo $path | head -1 2>/dev/null) ]]; then
            APP=$(echo $path | head -1)
            break
        fi
    done
